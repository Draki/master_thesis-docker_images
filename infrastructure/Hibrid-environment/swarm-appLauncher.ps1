...
docker-machine ssh $managerZero "docker exec $hadoopContainer /usr/local/hadoop/bin/hdfs dfs -put /usr/local/hadoop/data/DelightingCustomersBDextract2Formatted.json /data"
...
docker-machine ssh $managerZero """docker exec $sparkContainer spark-submit --class "thesisApp.ThesisAppLauncher" --deploy-mode client --master spark://spark-master:7077 app/thesisapp_2.11-1.0.jar "hdfs://hadoop-master:9000" "/data/" "DelightingCustomersBDextract2Formatted.json" "/results/" "app/dataExplorer_sample" "app/dataExplorer_sample2" """
