...
#
#  powershell -executionpolicy bypass -File .\infrastructure\HypervVMs-environment\swarm-appLauncher.ps1
##
...
echo "docker-machine ssh $managerZero `"docker exec $sparkContainer spark-submit --class `"thesisApp.ThesisAppLauncher`" --deploy-mode client --master spark://spark-master:7077 --executor-memory 650m ./app/$application `"hyperv`" $appConfigs "
echo ""
